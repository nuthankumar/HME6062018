    ...
    if (reportRowData.length > 0) {
        ...
        return (
          <tr>
            ...
            <td className={this.dynamicColumnData.showGroupsStores}> <a href='#' className='' onClick={() => this.props.handleDrillDown(reportItem.deviceId.value)}>{reportItem.store ? reportItem.store.name : ''} </a></td>
            <td className={'timeMeasureColumn ' + this.dynamicColumnData.showDayColumn} onClick={() => this.props.handleDrillDown(reportItem.deviceId.value)}><span className='timeSpan'>{reportItem.day? reportItem.day.timeSpan : '' }</span><br/><span className='currentMeasure'>{reportItem.day  ? reportItem.day.currentDaypart :''}</span></td>
